...
public class PicServiceImpl implements PicService {
    ...
    @Autowired
    public PicServiceImpl(OssUtil ossUtil, PicMapper picMapper) {
        this.ossUtil = ossUtil;
        this.picMapper = picMapper;
    }

    @Override
    public String insertPics(List<MultipartFile> files, Integer catId) {
        ...
        for (MultipartFile file : files) {
            ...
            Future<String> submit = executorService.submit(() -> {
                LOGGER.info("开始上传图片");
                String uploadUrl = ossUtil.upload(file);
                return uploadUrl;
            });
            futures.add(submit);
        }
